...
export function createSkillTool(skillLoader: SkillLoader): ToolDefinition {
  ...
  return {
    ...
    parametersForContext: (context) => ({
      type: "object",
      properties: {
        name: {
          type: "string",
          enum: context.skillView
            .listModelInvocable()
    ...
    execute: async (
      ...
      const skillView = executionContext?.skillView ?? skillLoader.createView();
      const skill = skillView.get(name);
      ...
      return [
        ...
        "",
        skillView.buildSkillContent(skill, args),
      ].join("\n");
    },
  };
}
